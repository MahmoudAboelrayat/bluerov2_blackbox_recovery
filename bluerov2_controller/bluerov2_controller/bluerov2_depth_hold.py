...
class Bluerov2DepthControl(LifecycleNode):
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        self._output_pub = self.create_publisher(OverrideRCIn, output_topic, 10)
    ...
    def _control_loop(self):
        ...
        out,dic = self._pid.update(self.goal_depth, self._current_depth,feedforward=self.flotability/10) 
        force = -out
        self.get_logger().info(f"PID debug: {dic}")
        pwm = self._force_to_pwm(force*10 / 4)

        msg_override = OverrideRCIn()
        msg_override.channels[0] = 1500  # pitch
        msg_override.channels[1] = 1500  # roll
        msg_override.channels[2] = int(pwm)  # heave
        msg_override.channels[3] = 1500  # yaw
        msg_override.channels[4] = 1500  # surge
        msg_override.channels[5] = 1500  # sway
        msg_override.channels[6] = 1500  # camera pan
        msg_override.channels[7] = 1500  # camera tilt

        self._output_pub.publish(msg_override)
